import { useAppStore } from "@/stores/app-store";
import { formatDuration } from "@/lib/utils";
import { useActiveTimer, useStopTimer, useStartTimer } from "@/hooks/use-timer";
import { Timer, Square, Search, Settings, ZoomIn, ZoomOut, Sun, Moon, X } from "lucide-react";

export function Header() {
  const { currentView, setView, zoomLevel, zoomIn, zoomOut, resetZoom, theme, toggleTheme, timerSuggestion, setTimerSuggestion } = useAppStore();
  const { data: activeTimer } = useActiveTimer();
  const stopTimerMutation = useStopTimer();
  const startTimerMutation = useStartTimer();

  const handleStopTimer = () => {
    stopTimerMutation.mutate();
  };

  const handleAcceptTimerSuggestion = () => {
    if (timerSuggestion) {
      startTimerMutation.mutate(timerSuggestion.projectName);
      setTimerSuggestion(null);
    }
  };

  return (
    <header className="shrink-0">
      <div
        className="h-12 border-b border-border flex items-center justify-between px-4"
        data-tauri-drag-region
      >
        <div className="flex items-center gap-4" data-tauri-drag-region>
          <div className="w-[70px]" />
          <h1 className="text-sm font-semibold" data-tauri-drag-region>
            {currentView === "today" && "Today"}
            {currentView === "projects" && "Projects"}
            {currentView === "settings" && "Settings"}
            {currentView === "project-detail" && "Project"}
            {currentView === "create-project" && "New Project"}
            {currentView === "disk-manager" && "Disk Manager"}
            {currentView === "port-manager" && "Port Manager"}
            {currentView === "permissions" && "Permissions"}
          </h1>
        </div>

        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 border-r border-border pr-2 mr-1">
            <button
              onClick={toggleTheme}
              className="p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-foreground"
              title={theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}
            >
              {theme === "dark" ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
            </button>
          </div>
          <div className="flex items-center gap-1 border-r border-border pr-2 mr-1">
            <button
              onClick={zoomOut}
              disabled={zoomLevel <= 50}
              className="p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed"
              title="Zoom out (Cmd+-)"
            >
              <ZoomOut className="h-3.5 w-3.5" />
            </button>
            <button
              onClick={resetZoom}
              className="px-1.5 py-0.5 text-xs font-mono text-muted-foreground hover:text-foreground hover:bg-secondary rounded min-w-[40px] text-center"
              title="Reset zoom"
            >
              {zoomLevel}%
            </button>
            <button
              onClick={zoomIn}
              disabled={zoomLevel >= 150}
              className="p-1.5 rounded hover:bg-secondary text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed"
              title="Zoom in (Cmd++)"
            >
              <ZoomIn className="h-3.5 w-3.5" />
            </button>
          </div>
        </div>

        <div className="flex items-center gap-3">
          {activeTimer && (
            <div className="flex items-center gap-2 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-3 py-1 rounded-full text-sm">
              <Timer className="h-3.5 w-3.5" />
              <span className="font-mono">
                {formatDuration(activeTimer.elapsed_seconds)}
              </span>
              <span className="text-green-600/70 dark:text-green-500/70">
                {activeTimer.project_name}
              </span>
              <button
                onClick={handleStopTimer}
                className="ml-1 p-0.5 hover:bg-green-200 dark:hover:bg-green-800/50 rounded"
                title="Stop timer"
              >
                <Square className="h-3 w-3 fill-current" />
              </button>
            </div>
          )}

          <button className="p-2 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground">
            <Search className="h-4 w-4" />
          </button>

          <button
            onClick={() => setView("settings")}
            className={`p-2 rounded-md hover:bg-secondary ${
              currentView === "settings" ? "text-foreground" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            <Settings className="h-4 w-4" />
          </button>
        </div>
      </div>

      {/* Timer suggestion bar */}
      {timerSuggestion && !activeTimer && (
        <div className="h-8 bg-blue-50 border-b border-blue-200 dark:bg-blue-900/20 dark:border-b dark:border-blue-900/30 flex items-center justify-center gap-3 px-4">
          <span className="text-xs text-blue-600 dark:text-blue-400">
            {timerSuggestion.projectName} opened
          </span>
          <button
            onClick={handleAcceptTimerSuggestion}
            className="text-xs px-2 py-0.5 rounded bg-blue-600 text-white hover:bg-blue-500"
          >
            Start timer?
          </button>
          <button
            onClick={() => setTimerSuggestion(null)}
            className="text-blue-400/60 hover:text-blue-600 dark:text-blue-500/60 dark:hover:text-blue-400"
          >
            <X className="h-3 w-3" />
          </button>
        </div>
      )}
    </header>
  );
}
